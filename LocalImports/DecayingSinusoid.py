...
def baseline_component(params, x):
    bl_pars = [params[key].value for key in list(params.keys()) if
               key.startswith('bl')]
...
class DecayingSinusoid(object):
    ...
    def _calculate_averaged_parameters(self):
        ...
        param_keys = [list(model.params.keys()) for model in self.models]
...
class ModelAveragedParameter(object):
    def __init__(self, key, models, weights):
        ...
        param_keys = [list(model.params.keys()) for model in models]
